...
> One animated component. A living constellation that maps my **works** and **studies**.
> Pure HTML + CSS + JavaScript — zero dependencies, zero build step, drop-in and go.
...
A single `<canvas>` component renders every project and study milestone as a **glowing node** in a drifting constellation. Nodes link by proximity, magnetize toward your cursor, pulse gently, and expand into a detail card on click.
...
├── style.css       ← styling
├── main.js         ← the animated component
└── data.js         ← edit ONLY this to change content
```
...
```bash
git clone https://github.com/PritulKumarRoy/magic-repo.git
cd magic-repo
# Just open index.html — no build, no install.
```

**Publish on GitHub Pages:**
1. Push the repo to GitHub.
2. **Settings → Pages → Source: `main` / `(root)`**.
3. Live at `https://pritulkumarroy.github.io/magic-repo/`.
...
```javascript
/* ============================================================
   EDIT ONLY THIS FILE TO UPDATE THE PORTFOLIO CONTENT
   ============================================================ */

const PROFILE = {
  name: "Pritul Kumar Roy",
  tagline: "ECE · Flutter Dev · UI/UX · RTL Design",
  bio: "B.Tech ECE graduate from Dr. B.C. Roy Engineering College, Durgapur. Creative head enthusiast exploring versatile corners of technology and creativity.",
  cgpa: "8.03",
  links: {
    github: "https://github.com/PritulKumarRoy",
    linkedin: "https://www.linkedin.com/in/pritul-kumar-roy-84a50a328/"
  }
};

// Every entry below becomes an animated node in the constellation.
const NODES = [
  // ---------- WORKS ----------
  ...
  { id: "geo", label: "Smart GNSS 'Geo'", type: "work",
    desc: "Android application performing geofencing, real-time location tracking, location sharing, map navigation and coordinate capture.",
  ...
  { id: "openroad", label: "OpenROAD ASIC · IIT Guwahati", type: "study",
    desc: "RTL-to-GDSII fully automated flow workshop by IIT Guwahati in association with MeitY. ASIC prototyping methodology.",
...
<head>
  ...
  <meta name="description" content="Pritul Kumar Roy — works and studies mapped as a living constellation." />
...
```javascript
/* ============================================================
   CONSTELLATION PORTFOLIO — the single animated component
   ------------------------------------------------------------
   • Nodes drift, link by proximity, magnetize toward the cursor
   • Hover to magnify, click to open a detail card
   • Fully responsive, DPR-aware, honors prefers-reduced-motion
   ============================================================ */
...
(() => {
  ...
  window.addEventListener('resize', () => {
    resize();
    // Keep nodes inside the new viewport.
    nodes.forEach(n => {
  ...
  const nodes = NODES.map((n, i) => {
    ...
    return {
      ...
      hovered: false,
      revealProgress: 0 // fade-in on load
    };
  });
  ...
  function draw() {
    ...
    // --- update node physics ---
    for (const n of nodes) {
      if (n.revealProgress < 1) n.revealProgress += 0.02;
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        ...
        if (d < LINK_DIST) {
          const baseAlpha = (1 - d / LINK_DIST) * 0.35;
          const alpha = baseAlpha * Math.min(a.revealProgress, b.revealProgress);
          if (alpha <= 0) continue;
    ...
    for (const n of nodes) {
      ...
      const r = (n.hovered ? baseR + 4 : baseR) * n.revealProgress;
  ...
  // ---------- Boot ----------
  resize();
  // Recompute node positions after we know real dimensions
  nodes.forEach((n, i) => {
    const angle  = (i / NODES.length) * Math.PI * 2;
...
|---|---|
| Add / remove a project or study | `data.js` → `NODES` |
| Node colors | `data.js` → `COLORS` |
| Link distance between nodes | `main.js` → `LINK_DIST` |
| Cursor magnet strength | `main.js` → `0.03` inside "cursor magnet" |
| Drift speed | `data.js`-driven via `vx`/`vy` init in `main.js` |
| Header text | `data.js` → `PROFILE` |

---

## ♿ Accessibility & Quality

- **`prefers-reduced-motion`** — freezes all drift, keeps the scene static and usable.
- **Keyboard** — `Esc` closes the detail card.
- **Touch** — full support: drag to attract, tap a node to open its card.
- **DPR-aware** — crisp on retina and high-density displays.
- **Zero dependencies** — no framework, no bundler, no CDN.
